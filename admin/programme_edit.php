...
/**
 * ================================================
 * INVESTHOOD IT - Edit Programme Page
 * ================================================
 * Admin-only programme edit form.
 */
...
<head>
  ...
  <meta name="description" content="Edit Programme - Investhood IT Administrator">
  <title>Edit Programme | Investhood IT Admin</title>
    ...
    <main class="dashboard__main">
      <header class="dash-header admin-dash-header">
        <div class="dash-header__left">
          ...
          <a href="<?= url('admin/programme_detail.php?id=' . $id) ?>" class="btn btn--ghost btn--sm"><i class="fas fa-arrow-left"></i> Back</a>
        </div>
      ...
      <div class="dash-content">
        <section class="pm-hero">
          <div class="pm-hero__inner">
            <div class="pm-hero__text">
              ...
              <h1 class="pm-hero__title">Edit: <?= e($programme['name']) ?></h1>
              <p class="pm-hero__subtitle">Update the details of this programme.</p>
            </div>
        ...
        <form method="post" action="<?= url('admin/programme_actions.php') ?>" class="pm-form" novalidate>
            ...
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field pm-form-field--full">
                  ...
                  <input type="text" id="name" name="name" class="form-input" value="<?= e($old['name'] ?? $programme['name']) ?>" required>
                ...
                <div class="pm-form-field">
                  ...
                  <select id="type" name="type" class="form-input form-input--select">
                    <?php foreach (PROGRAMME_TYPE_LABELS as $slug => $label): ?>
                      <option value="<?= e($slug) ?>" <?= ($old['type'] ?? $programme['type']) === $slug ? 'selected' : '' ?>><?= e($label) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="pm-form-field">
                  <label class="pm-form-label" for="duration">Programme Duration</label>
                  <input type="text" id="duration" name="duration" class="form-input" value="<?= e($old['duration'] ?? $programme['duration']) ?>" placeholder="e.g. 12 months">
                </div>
              </div>
              <div class="pm-form-row">
                <div class="pm-form-field pm-form-field--full">
                  <label class="pm-form-label" for="description">Description <span class="pm-req">*</span></label>
                  <textarea id="description" name="description" class="form-input" rows="4" required><?= e($old['description'] ?? $programme['description'] ?? '') ?></textarea>
                </div>
              </div>
              <div class="pm-form-row">
                <div class="pm-form-field pm-form-field--full">
                  <label class="pm-form-label" for="objectives">Programme Objectives</label>
                  <textarea id="objectives" name="objectives" class="form-input" rows="4" placeholder="Outline the key objectives of this programme..."><?= e($old['objectives'] ?? $programme['objectives'] ?? '') ?></textarea>
                </div>
              </div>
            </div>
          </section>
            ...
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field">
                  ...
                  <input type="date" id="start_date" name="start_date" class="form-input" value="<?= e($old['start_date'] ?? $programme['start_date'] ?? '') ?>">
                </div>
                <div class="pm-form-field">
                  <label class="pm-form-label" for="end_date">Programme End Date</label>
                  <input type="date" id="end_date" name="end_date" class="form-input" value="<?= e($old['end_date'] ?? $programme['end_date'] ?? '') ?>">
                </div>
              </div>
          ...
          <section class="pm-form-section">
            <div class="pm-form-section__header"><span class="pm-form-section__icon"><i class="fas fa-flag"></i></span><div><h3>Status</h3><p>Programme lifecycle status. Transitions are restricted by business rules.</p></div></div>
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field">
                  ...
                  <select id="status" name="status" class="form-input form-input--select">
                    <?php foreach (PROGRAMME_STATUS_LABELS as $slug => $label): ?>
                      <option value="<?= e($slug) ?>" <?= ($old['status'] ?? $programme['status']) === $slug ? 'selected' : '' ?>><?= e($label) ?></option>
                    <?php endforeach; ?>
                  </select>
          ...
          <div class="pm-form-actions">
            <a href="<?= url('admin/programme_detail.php?id=' . $id) ?>" class="btn btn--ghost"><i class="fas fa-times"></i> Cancel</a>
