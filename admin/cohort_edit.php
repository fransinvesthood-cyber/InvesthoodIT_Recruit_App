...
/**
 * ================================================
 * INVESTHOOD IT - Edit Cohort Page
 * ================================================
 * Admin-only cohort edit form.
 */
...
<head>
  ...
  <meta name="description" content="Edit Cohort - Investhood IT Administrator">
  <title>Edit Cohort | Investhood IT Admin</title>
  ...
  <div class="dashboard">
    ...
    <main class="dashboard__main">
      <header class="dash-header admin-dash-header">
        <div class="dash-header__left"><button class="dash-header__toggle" id="sidebarToggle" aria-label="Toggle sidebar"><i class="fas fa-bars"></i></button><a href="<?= url('admin/cohort_detail.php?id=' . $id) ?>" class="btn btn--ghost btn--sm"><i class="fas fa-arrow-left"></i> Back</a></div>
      ...
      <div class="dash-content">
        <section class="pm-hero">
          <div class="pm-hero__inner">
            <div class="pm-hero__text">
              ...
              <h1 class="pm-hero__title">Edit: <?= e($cohort['name']) ?></h1>
              <p class="pm-hero__subtitle">Update cohort details. Capacity cannot be reduced below committed participants.</p>
            </div>
        ...
        <form method="post" action="<?= url('admin/cohort_actions.php') ?>" class="pm-form" novalidate>
            ...
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field">
                  ...
                  <select id="programme_id" name="programme_id" class="form-input form-input--select" required>
                    <?php foreach ($programmes as $p): ?>
                      <option value="<?= (int) $p['id'] ?>" <?= (int)($old['programme_id'] ?? $cohort['programme_id']) === (int)$p['id'] ? 'selected' : '' ?>><?= e($p['name']) ?></option>
                    <?php endforeach; ?>
                  </select>
                ...
                <div class="pm-form-field">
                  ...
                  <input type="text" id="name" name="name" class="form-input" value="<?= e($old['name'] ?? $cohort['name']) ?>" required>
                </div>
              </div>
              <div class="pm-form-row">
                <div class="pm-form-field pm-form-field--full">
                  <label class="pm-form-label" for="description">Cohort Description</label>
                  <textarea id="description" name="description" class="form-input" rows="3"><?= e($old['description'] ?? $cohort['description'] ?? '') ?></textarea>
                </div>
              </div>
            </div>
          </section>
          ...
          <section class="pm-form-section">
            <div class="pm-form-section__header"><span class="pm-form-section__icon"><i class="fas fa-calendar-alt"></i></span><div><h3>Dates &amp; Applications</h3></div></div>
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field"><label class="pm-form-label" for="start_date">Start Date</label><input type="date" id="start_date" name="start_date" class="form-input" value="<?= e($old['start_date'] ?? $cohort['start_date'] ?? '') ?>"><?= field_error($errors, 'start_date') ?></div>
                <div class="pm-form-field"><label class="pm-form-label" for="end_date">End Date</label><input type="date" id="end_date" name="end_date" class="form-input" value="<?= e($old['end_date'] ?? $cohort['end_date'] ?? '') ?>"><?= field_error($errors, 'end_date') ?></div>
              </div>
              <div class="pm-form-row">
                <div class="pm-form-field"><label class="pm-form-label" for="application_open_date">Application Opening Date</label><input type="date" id="application_open_date" name="application_open_date" class="form-input" value="<?= e($old['application_open_date'] ?? $cohort['application_open_date'] ?? '') ?>"><?= field_error($errors, 'application_open_date') ?></div>
                <div class="pm-form-field"><label class="pm-form-label" for="application_close_date">Application Closing Date</label><input type="date" id="application_close_date" name="application_close_date" class="form-input" value="<?= e($old['application_close_date'] ?? $cohort['application_close_date'] ?? '') ?>"><?= field_error($errors, 'application_close_date') ?></div>
              </div>
            ...
            <div class="pm-form-section__body">
              <div class="pm-form-row">
                <div class="pm-form-field">
                  ...
                  <input type="number" id="max_capacity" name="max_capacity" class="form-input" min="1" value="<?= e($old['max_capacity'] ?? $cohort['max_capacity']) ?>" required>
                ...
                <div class="pm-form-field">
                  ...
                  <select id="delivery_mode" name="delivery_mode" class="form-input form-input--select">
                    <?php foreach (COHORT_DELIVERY_LABELS as $slug => $label): ?>
                      <option value="<?= e($slug) ?>" <?= ($old['delivery_mode'] ?? $cohort['delivery_mode']) === $slug ? 'selected' : '' ?>><?= e($label) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
              <div class="pm-form-row">
                <div class="pm-form-field">
                  ...
                  <input type="text" id="location" name="location" class="form-input" value="<?= e($old['location'] ?? $cohort['location'] ?? '') ?>" placeholder="e.g. Sandton Campus">
                </div>
                <div class="pm-form-field">
                  ...
                  <select id="province" name="province" class="form-input form-input--select">
                    ...
                    <?php foreach (ALLOWED_PROVINCES as $pr): ?>
                      <option value="<?= e($pr) ?>" <?= ($old['province'] ?? $cohort['province'] ?? '') === $pr ? 'selected' : '' ?>><?= e(ucwords(str_replace('-', ' ', $pr))) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
                ...
                <div class="pm-form-field">
                  ...
                  <select id="status" name="status" class="form-input form-input--select">
                    <?php foreach (COHORT_STATUS_LABELS as $slug => $label): ?>
                      <option value="<?= e($slug) ?>" <?= ($old['status'] ?? $cohort['status']) === $slug ? 'selected' : '' ?>><?= e($label) ?></option>
                    <?php endforeach; ?>
                  </select>
          ...
          <div class="pm-form-actions">
            <a href="<?= url('admin/cohort_detail.php?id=' . $id) ?>" class="btn btn--ghost"><i class="fas fa-times"></i> Cancel</a>
